...
class Part2 {
    public record GeoPoint(double latitude, double longitude){
        ...
        public GeoPoint average(GeoPoint other) {
            return new GeoPoint((latitude + other.latitude) / 2, (longitude + other.longitude) / 2);
        }
    }
    public record GeoBox(GeoPoint ne, GeoPoint sw){
        public GeoBox {
            if (ne.latitude() < sw.latitude() || ne.longitude() < sw.longitude()) {
                throw new IllegalArgumentException("ne must be north and east of sw");
            }
            if (ne.latitude() - sw.latitude() > 10 || ne.longitude() - sw.longitude() > 10) {
                throw new IllegalArgumentException("latitude and longitude must be within 10 degrees");
            }
            if (ne.latitude() > 80 || sw.latitude() < -80) {
                throw new IllegalArgumentException("latitude must be within 80 degrees of the equator");
            }
            if (ne.latitude() - sw.latitude() > 5 || ne.longitude() - sw.longitude() > 5) {
                throw new IllegalArgumentException("latitude and longitude must be within 5 degrees");
            }
            if (ne.longitude() - sw.longitude() > 5) {
                if (ne.longitude() - sw.longitude() > 180) {
                    throw new IllegalArgumentException("longitude must be within 5 degrees");
                }
                if (ne.longitude() < 0 && sw.longitude() > 0) {
                    throw new IllegalArgumentException("longitude must be within 5 degrees");
                }
            }
        }
