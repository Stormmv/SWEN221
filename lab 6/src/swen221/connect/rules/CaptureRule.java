...
public class CaptureRule implements Rule {
	...
	private boolean isCaptured(Board original, Board.Token token, int x, int y) {
		return isHorizontalLongCapture(original, token, x, y) || 
				isHorizontalShortCapture(original, token, x, y) || 
				isVerticalLongCapture(original, token, x, y) || 
				isVerticalShortCapture(original, token, x, y);
	}
